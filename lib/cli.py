    ...
    while True:
        ...
        if choice == '1':
            while True:
                ...
                if w_choice == '1':
                    try:
                        # Prompt for number of workouts (1-4)
                        num_workouts = validate_positive_int(input("How many workouts to create (1-4)? "), "number of workouts")
                        if num_workouts < 1 or num_workouts > 4:
                            raise ValueError("Number of workouts must be between 1 and 4")
